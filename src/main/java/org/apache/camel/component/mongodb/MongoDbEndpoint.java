 ...
 */
@UriEndpoint(firstVersion = "2.19.0", scheme = "mongodb", title = "MongoDB", syntax = "mongodb:connectionBean", consumerClass = MongoDbTailableCursorConsumer.class, category = {org.apache.camel.Category.DATABASE, org.apache.camel.Category.NOSQL})
public class MongoDbEndpoint extends DefaultEndpoint {
    ...
    public void initializeConnection() throws CamelMongoDbException {
        ...
        if (collection != null) {
            ...
            try {
                db.listCollectionNames().into(new java.util.ArrayList<>());
                ...
                throw new CamelMongoDbException("Error checking/creating collection", e);
            }
            
            dbCollection = db.getCollection(collection, Document.class);
    ...
    public void ensureIndex(MongoCollection<Document> collection, List<Document> dynamicIndex) {
        if (dynamicIndex != null && !dynamicIndex.isEmpty()) {
            for (Document index : dynamicIndex) {
                ...
                collection.createIndex(Indexes.ascending(index.keySet().toArray(new String[0])));
            }
        }
    }
